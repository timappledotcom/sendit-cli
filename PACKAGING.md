...
```bash
fpm -s dir -t deb -n sendit-cli -v 1.0.0 \
...
```bash
fpm -s dir -t pacman -n sendit-cli -v 1.0.0 \
  --description "CLI tool for posting to Micro.blog and X simultaneously" \
  --url "https://github.com/timappledotcom/sendit-cli" \
  --maintainer "SendIt CLI <noreply@example.com>" \
  --license MIT \
...
```bash
tar -czf sendit-cli-1.0.0-linux-x86_64.tar.gz bin lib Gemfile README.md LICENSE install.sh
```
...
docker run --rm -v $(pwd):/workspace -w /workspace rockylinux:9 bash -c "
  ...
  cp sendit-cli-1.0.0-linux-x86_64.tar.gz ~/rpmbuild/SOURCES/ &&
...
### Test Debian Package
```bash
dpkg -c sendit-cli_1.0.0_amd64.deb
```

### Test Arch Package
```bash
tar -tzf sendit-cli-1.0.0-1-x86_64.pkg.tar.zst
# Check the executable:
tar -xOf sendit-cli-1.0.0-1-x86_64.pkg.tar.zst usr/local/bin/scli | head -10
```

### Test RPM Package
```bash
rpm -qlp sendit-cli-1.0.0-1.el9.x86_64.rpm
```
...
### Issue: RPM build fails with "cd: sendit-cli-1.0.0-linux-x86_64: No such file or directory"
